...
class _CalendarJs {
  ...
  final lunarInfo = [
    ...
    0x06566, 0x0d4a0, 0x0ea50, 0x16a95, 0x05ad0, 0x02b60, 0x186e3, 0x092e0,
  ...
  /// 公历每个月份的天数普通表
  /// @Array Of Property
  final solarMonth = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31];
  ...
  String toAstro(int cMonth, int cDay) {
    ...
    return '${s.substring(start, start + 2)}\u5ea7'; // 座
  }
  ...
  int getTerm(int y, int n) {
    ...
    for (var index = 0; index < table.length; index += 5) {
      var x = table.substring(index, index + 5);
      var chunk = int.parse('0x$x').toString();

      calcDay.add(chunk[0]);
      calcDay.add(chunk.substring(1, 1 + 2));
      calcDay.add(chunk[3]);
      calcDay.add(chunk.substring(4, 4 + 2));
    }

    return int.parse(calcDay[n - 1]);
  }
  ...
  lunar2solar(int y, int m, int d, bool isLeapMonth) {
    ...
    var calObj = Date((offset + d - 31) * 86400000 + strap);
...
class CalendarUtils {
  static final _converter = _CalendarJs();

  static solar2lunar(int y, int m, int d) {
    return _converter.solar2lunar(y, m, d);
  }

  static lunar2solar(int y, int m, int d, [bool isLeapMonth = false]) {
    return _converter.lunar2solar(y, m, d, isLeapMonth);
  }
}
